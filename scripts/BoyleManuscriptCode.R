...
#P2plot <- ggplot() + geom_point(data = P2model, aes(P2, drw), colour="#7200a3", alpha=0.5) +  
              geom_ribbon(data = P2predict, aes(x = x, ymin = conf.low, ymax = conf.high), fill = "lightgrey", alpha = 0.5) +
              theme_JB() + 
              labs(x = "Emergence (doy)", y = "Relative growth")
