...
setwd("/Users/joeboyle/Desktop/Blissertation")
...
# Zero amplitude / active layer data
temp_range_zero_amplitude <- read.csv("TpsData.csv") %>%
  dplyr::select(Year, Tps) %>% na.omit()
names(temp_range_zero_amplitude)[2] <- "t_stable"

# NDVI data
qiki_gimms <- read.csv("~/ShrubHub/scripts/users/jassmann/phenology/data/qiki_gimms.csv")
names(qiki_gimms)[2]<-"Year"
qiki_gimms <- qiki_gimms %>% dplyr::select(-X)
qiki_modis <- read.csv("~/ShrubHub/scripts/users/jassmann/phenology/data/qiki_modis.csv")
names(qiki_modis)[2]<-"Year"
qiki_modis <- qiki_modis %>% dplyr::select(-X)

# Sea ice data
seaice <- read.csv("~/ShrubHub/manuscripts/Qikiqtaruk_Manuscript/data/cwa01_02_rmin_1968_2016_0326_1015.csv")
...
# Add zero amp data
dendroclim2 <- merge(dendroclim2, temp_range_zero_amplitude, by = "Year", all = TRUE)

# Add gimms/modis data
dendroclim2 <- merge(qiki_gimms, dendroclim2, by = "Year", all = TRUE) %>%
  filter(Year > 1990)
dendroclim2 <- merge(qiki_modis, dendroclim2, by = "Year", all = TRUE)

# Add sea ice data
seaice <- seaice %>% dplyr::select(Season, Total.Concentration)
colnames(seaice)[1] <- "Year"
dendroclim2 <- merge(seaice, dendroclim2, by="Year", all = TRUE) %>%
  filter(Year > 1990)

# Select key variables
dendroclim2 <- dplyr::select(dendroclim2, Year, Plot, Individual, drw, rw, area, darea, count, Site,
                             P2, P5, Px, pPx, ptsummer, ptautumn, twinter, tspring, tsummer, tautumn, ppsummer, ppautumn, 
                             pwinter, pspring, psummer, pautumn, max.ndvi, gimms3g_ndvi, Total.Concentration, t_stable)
names(dendroclim2)[26] <- "NDVImodis"
names(dendroclim2)[27] <- "NDVIgimms"
names(dendroclim2)[28] <- "seaice_conc"


## Remove 2016 data (growth not complete)
dendroclim2 <- filter(dendroclim2, Year != 2016)

## Filter out years without full data for final AICs (Do you need all these?!)
dendroclimAIC <- filter(dendroclim2, Year > 2001 & Year < 2013)
...
dendroclimAICscaledlong <- gather(dendroclimAICscaled, Variable, Value, c(1:20))

## Filter out years without full data for p1 AICs (Do you need all these?!)
p1climAIC <- dendroclim2 %>% filter(Year > 2001) %>% 
  dplyr::select(Px, pPx, Year, Plot, Individual, drw, darea) %>%
  gather(Variable, Value, c(1:2))

## Filter out years without full data for p2 AICs (Do you need all these?!)
p2climAIC <- dendroclim2 %>% filter(Year > 2000, darea > 0) %>% 
  dplyr::select(P2, P5, Px, Year, Plot, Individual, drw, darea) %>%
  gather(Variable, Value, c(1:3))
...
## Pheno1 ----
## Pheno1 linear models
p1models <- p1climAIC %>% 
  group_by(Variable) %>%
  do(fit = lmer(darea ~ (Value) + (1|Plot) + (1|Individual) + (1|Year), data = ., REML = TRUE)) %>%
  tidy(fit) %>%
  filter(term == "Value")
rownames(p1models) <- (c("Previous growing season", "Growing season"))
p1models <- p1models %>% dplyr::select(estimate, std.error, statistic, Variable)

# AIC and Distributions of resids
p1results <- data.frame("AIC"=NA, "pseudo.R2m"=NA, "pseudo.R2c"=NA, "LR p"=NA, "LR X"=NA)[numeric(0), ]
p1AIC_var <- unique(p1climAIC$Variable)
p1var_names <- c("Growing season length", "Previous growing season length")
for (i in 1:length(p1AIC_var)) {
  p1modeldata <- p1climAIC[p1climAIC$Variable == p1AIC_var[i],]
  p1AIC_nullREML <- lmer(darea ~ 1 + (1|Plot) + (1|Individual) + (1|Year), data = p1modeldata, REML = TRUE)
  p1AIC_null <- lmer(darea ~ 1 + (1|Plot) + (1|Individual) + (1|Year), data = p1modeldata, REML = FALSE)
  p1AIC_modelREML <- lmer(darea ~ Value + (1|Plot) + (1|Individual) + (1|Year), data = p1modeldata, REML = TRUE)
  p1AIC_model <- lmer(darea ~ Value + (1|Plot) + (1|Individual) + (1|Year), data = p1modeldata, REML = FALSE)
  p1results[i+1,1] <- summary(p1AIC_model)$AIC[1]
  p1results[i+1,2] <- r.squaredGLMM(p1AIC_modelREML)[1]
  p1results[i+1,3] <- r.squaredGLMM(p1AIC_modelREML)[2]
  p1results[i+1,4] <- anova(p1AIC_model, p1AIC_null) [2,8]
  p1results[i+1,5] <- anova(p1AIC_model, p1AIC_null) [2,6]
  p1modelplot <- ggplot(p1AIC_modelREML, aes(Value, darea)) + geom_point() +  geom_smooth(method=lm) +
    theme_JB() + labs(x = p1var_names[i])
  diagplot <- ggplot(p1AIC_model, aes(.fitted, .resid))  + geom_point() + 
    geom_hline(yintercept = 0, alpha = 0.5, linetype = 3) +
    theme_JB() + labs(title = p1var_names[i])
  autocor <- acf(residuals(p1AIC_modelREML), plot = FALSE)
  ...
  grid.arrange(main=textGrob(p1var_names[i], gp=gpar(fontsize=20)), p1modelplot, diagplot, corplot)
  print(p1modelplot + labs (y = "Relative growth"))
}
p1results[1,1] <- summary(p1AIC_null)$AIC[1]
p1results[1,2] <- r.squaredGLMM(p1AIC_nullREML)[1]
p1results[1,3] <- r.squaredGLMM(p1AIC_nullREML)[2]
p1results[1,4] <- "-"
p1results[1,5] <- "-"
rownames(p1results) <- c("Null", "Growing season", "Previous growing season")

# Plot pheno1 figures
ggplot(p1models, aes(x = Variable, y = estimate)) + 
  geom_hline(yintercept=0, alpha=0.5, linetype = 3) +
  geom_crossbar(colour = "#7200a3", fill = "#7200a3", width = 0.7,
                aes(ymin = estimate - std.error, ymax = estimate + std.error, alpha = 0.8)) +
  theme_JB() + ylab("Effect Size") +
  theme(legend.position = "none", axis.title.x = element_blank()) +
  scale_x_discrete(limits=c("Px","pPx"), labels =  c("GSL", "Previous GSL"))

## Pheno2 ----
## Pheno2 linear models
p2models <- p2climAIC %>%
  group_by(Variable) %>%
  do(fit = lmer(darea ~ (Value) + (1|Plot) + (1|Individual) + (1|Year), data = ., REML = TRUE)) %>%
  tidy(fit) %>%
  filter(term == "Value")
rownames(p2models) <- (c("Emergence", "Senescence", "Growing season"))
p2models <- p2models %>% dplyr::select(estimate, std.error, statistic, Variable)

# AIC and Distributions of resids
p2results <- data.frame("AIC"=NA, "pseudo.R2m"=NA, "pseudo.R2c"=NA, "LR p"=NA, "LR X"=NA)[numeric(0), ]
p2AIC_var <- unique(p2climAIC$Variable)
p2var_names <- c("Emergence (doy)", "Senescence (doy)", "Growing season length")
for (i in 1:length(p2AIC_var)) {
  p2modeldata <- p2climAIC[p2climAIC$Variable == p2AIC_var[i],]
  p2AIC_nullREML <- lmer(darea ~ 1 + (1|Plot) + (1|Individual) + (1|Year), data = p2modeldata, REML = TRUE)
  p2AIC_null <- lmer(darea ~ 1 + (1|Plot) + (1|Individual) + (1|Year), data = p2modeldata, REML = FALSE)
  p2AIC_modelREML <- lmer(darea ~ Value + (1|Plot) + (1|Individual) + (1|Year), data = p2modeldata, REML = TRUE)
  p2AIC_model <- lmer(darea ~ Value + (1|Plot) + (1|Individual) + (1|Year), data = p2modeldata, REML = FALSE)
  p2results[i+1,1] <- summary(p2AIC_model)$AIC[1]
  p2results[i+1,2] <- r.squaredGLMM(p2AIC_modelREML)[1]
  p2results[i+1,3] <- r.squaredGLMM(p2AIC_modelREML)[2]
  p2results[i+1,4] <- anova(p2AIC_model, p2AIC_null) [2,8]
  p2results[i+1,5] <- anova(p2AIC_model, p2AIC_null) [2,6]
  p2modelplot <- ggplot(p2AIC_modelREML, aes(Value, darea)) + geom_point() +  geom_smooth(method=lm) +
    theme_JB() + labs(x = p2var_names[i])
  diagplot <- ggplot(p2AIC_model, aes(.fitted, .resid))  + geom_point() + 
    geom_hline(yintercept = 0, alpha = 0.5, linetype = 3) +
    theme_JB() + labs(title = p2var_names[i])
  autocor <- acf(residuals(p2AIC_modelREML), plot = FALSE)
  acfdf <- with(autocor, data.frame(lag, acf))
  corplot <- ggplot(data = acfdf, mapping = aes(x = lag, y = acf)) +
    geom_hline(aes(yintercept = 0)) +
    geom_segment(mapping = aes(xend = lag, yend = 0)) + theme_JB() # NEED TO ADD THE BLUE DASHED LINES ~0.145 intercept
  grid.arrange(main=textGrob(p2var_names[i], gp=gpar(fontsize=20)), p2modelplot, diagplot, corplot)
  print(p2modelplot + labs (y = "Relative growth"))
}
p2results[1,1] <- summary(p2AIC_null)$AIC[1]
p2results[1,2] <- r.squaredGLMM(p2AIC_nullREML)[1]
p2results[1,3] <- r.squaredGLMM(p2AIC_nullREML)[2]
p2results[1,4] <- "-"
p2results[1,5] <- "-"
rownames(p2results) <- c("Null", "Emergence", "Senescence", "Growing season")

# Plot pheno2 figures
ggplot(p2models, aes(x = Variable, y = estimate)) + 
  geom_hline(yintercept=0, alpha=0.5, linetype = 3) +
  geom_crossbar(colour = "#7200a3", fill = "#7200a3", width = 0.7,
                aes(ymin = estimate - std.error, ymax = estimate + std.error, alpha = 0.8)) +
  theme_JB() + ylab("Effect Size") +
  theme(legend.position = "none", axis.title.x = element_blank()) +
  scale_x_discrete(limits=c("P2","P5","Px"), labels =  c("Emergence", "Senescence", "GSL"))
...
models1scaled <- dendroclimAICscaledlong %>%
  ...
  tidy(fit) %>%
  filter(term == "Value")
rownames(models1scaled) <- (c("GIMMS NDVI", "MODIS NDVI", "Emergence", "Senescence", "Autumn precipitation", "Previous autumn precipitation",
                              ...
                              "Winter precipitation", "Growing season", "Sea ice concentration", "Permafrost surface temperature",
...
var_names <- c("Emergence", "Senescence", "Growing season", "Previous growing season", "Previous summer temperature", 
               ...
               "GIMMS NDVI", "MODIS NDVI", "Sea ice concentration", "Permafrost surface temperature")
...
# Plot overall figure
groupings_order <- c(rep("ndvi", 2), rep("pheno", 2), rep("precip", 3), "pheno", 
                     rep("precip", 2), rep("temp", 2), "precip", "pheno", "ice", "soil",
                     rep("temp", 4))
...
ggplot(models1scaled, aes(colour = type, fill = type, x = Variable, y = estimate)) + 
  ...
  scale_x_discrete(limits=c("P2","P5","Px","pPx","ptsummer","ptautumn","twinter","tspring","tsummer",
                            ...
                            "NDVIgimms","NDVImodis","seaice_conc","t_stable"), 
                   labels =  c("Emergence", "Senescence", "GSL", "Previous GSL", expression(paste("Previous T"[summer])), 
                               ...
                               "GIMMS NDVI", "MODIS NDVI", "Sea ice", expression(paste("T"[ps])))) +
  scale_fill_manual(values = c("#65c0ed","#F2AD00","#7200a3","#00A08A","#ce0000","#F98400")) +
  scale_colour_manual(values = c("#65c0ed","#F2AD00","#7200a3","#00A08A","#ce0000","#F98400")) 
